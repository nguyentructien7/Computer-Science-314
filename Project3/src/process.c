...
//Power Function to replace math.h's implementation
...
ProcessSchedule ()
{
  ...
  currentPCB->estcpu = currentPCB->estcpu + 1;
  currentPCB->priority = PUSER + (currentPCB->estcpu/4) + 2*currentPCB->p_nice;
...
// because it can't be done while the process might still be running          
  ...
  for (j = 0; j < 32; j++){
    // Now, run the one with the lowest priority
    for (i = 0; i < QueueLength (&(runQueue[j])); i++){
      pcb = (PCB *)((QueueFirst (&(runQueue[j])))->object);
      QueueRemove( &pcb->l);
      QueueInsertLast( &(runQueue[(int)pcb->priority/4]), &pcb->l);
	  if (pcb != NULL && pcb != 0 && pcb->priority >= PUSER && !(pcb->flags & PROCESS_STATUS_ZOMBIE) && !(pcb->flags & PROCESS_STATUS_WAITING) && (pcb->flags & PROCESS_STATUS_RUNNABLE)) {
      	if (currentPCB == 0 || currentPCB == NULL || pcb->priority < currentPCB->priority){
	  	  currentPCB = pcb;
		}
      }
    }
  ...
  pcb = currentPCB;
  if (currentPCB == NULL || currentPCB == 0){
...
ProcessWakeup (PCB *wakeup)
{
  ...
  if ((wakeup->sleepTime - My_TimerGet()) >= (uint32)1000){
    wakeup->estcpu = (wakeup->estcpu) * power((2*LOAD)/(2*LOAD + 1), (wakeup->sleepTime)/processQuantum);
    wakeup->priority = PUSER + (wakeup->estcpu/4) + 2*wakeup->p_nice;
  }
...
ProcessFork (VoidFunc func, uint32 param, int p_nice, int p_info,char *name, int isUser)
{
  ...
  pcb->sleepTime = 0;
  pcb->p_nice = p_nice;
  pcb->priority = PUSER + (pcb->estcpu/4) + 2*pcb->p_nice;
  ...
  intrs = DisableIntrs ();
  QueueInsertLast (&runQueue[(int)(pcb->priority / 4)], l);
  RestoreIntrs (intrs);
